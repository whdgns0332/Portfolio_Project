...
namespace SmartFactoryProject_Final.Common
{
    class ControlLayout
    {
        ...
        public void SetBackgroundImage(Control ctrl, string image)
        {
            try
            {
                ...
                ctrl.BackgroundImage = Image.FromFile(image);
            }
            catch (FileNotFoundException excep)
            {
                string className = nameof(ControlLayout);
                string funcName = nameof(SetBackgroundImage);
        ...
        public void SetImage(PictureBox picture, string image)
        {
            ...
            {
                string className = nameof(ControlLayout);
                string funcName = nameof(SetImage);
                string logText = string.Concat(excep.Message.ToString(), Environment.NewLine,
                                               $"이미지 {image}가 존재하지 않습니다");
                Log.WriteLog(Log.LogType.Error, className, funcName, logText);
            }
    ...
    public static class ControlExtend    // 컨트롤 확장용 클래스
    {
        public static void DefaultSetting(this Button btn)
        {
            btn.Text = "";
            btn.TabStop = false;
        ...
        public static void DefaultSetting(this TextBox txt)
        {
            txt.Text = "";
            txt.Multiline = true;
            txt.AutoSize = true;
